...
class Dia2:
    ...
    def generate_stream(
        ...
        **overrides,
        ...
        """
        Generate audio with streaming, yielding chunks as they're generated.

        This method yields StreamEvent objects during generation, allowing
        real-time audio streaming to clients.

        Args:
            script: The text script to generate audio from
            config: Optional generation configuration
            streaming_config: Optional streaming configuration
            prefix_speaker_1: Optional voice prompt for speaker 1
            prefix_speaker_2: Optional voice prompt for speaker 2
        ...
        Yields:
            ...
            - AudioChunkEvent: Audio data chunks (~0.1s each)
            - CompleteEvent: Generation complete
            - ErrorEvent: On errors
        """
        runtime = self._ensure_runtime()
        logger = RuntimeLogger(verbose)
